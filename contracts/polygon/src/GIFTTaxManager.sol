...
contract GIFTTaxManager is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /// @dev Architecture doc says inbound currently exempt; keep it global-by-default.
    ...
    function setTaxOfficer(address _newTaxOfficer) external onlyOwner {
        require(_newTaxOfficer != address(0), "GIFTTaxManager: Cannot set tax officer to address zero");
        taxOfficer = _newTaxOfficer;
        emit NewTaxOfficer(taxOfficer);
    }

    function setBeneficiary(address _newBeneficiary) external onlyOwner {
        require(_newBeneficiary != address(0), "GIFTTaxManager: Cannot set beneficiary to address zero");
    ...
    function setLiquidityPool(address _liquidityPool, bool _isPool) external onlyOwner {
