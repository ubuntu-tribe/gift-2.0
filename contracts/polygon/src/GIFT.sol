    ...
    UUPSUpgradeable
{
    ...
    modifier onlySupplyManager() {
        require(msg.sender == supplyManager, "GIFT: Caller is not the supply manager");
        _;
    }
    ...
    function computeTaxUsingManager(uint256 _transferAmount) internal view returns (uint256) {
        ...
        (
            ...
            uint256 tierFourMax
        ) = taxManager.getTaxTiers();
        (
            uint256 tierOneTax,
            uint256 tierTwoTax,
            uint256 tierThreeTax,
            uint256 tierFourTax,
            uint256 tierFiveTax
        ) = taxManager.getTaxPercentages();
    ...
    function delegateTransferProof(
        ...
        return
            keccak256(abi.encodePacked(getChainID(), token, amount, delegator, spender, networkFee));
    }

    function setSupplyController(address _newSupplyController) external onlyOwner {
        require(_newSupplyController != address(0), "GIFT: Cannot set supply controller to address zero");
        supplyController = _newSupplyController;
        emit NewSupplyController(supplyController);
    }

    function setSupplyManager(address _newSupplyManager) external onlyOwner {
        require(_newSupplyManager != address(0), "GIFT: Cannot set supply manager to address zero");
    ...
    function inflateSupply(uint256 _value) external onlySupplyManager returns (bool) {
        _mint(supplyManager, _value);
        return true;
    }

    function increaseSupply(address _userAddress, uint256 _value) external onlySupplyController returns (bool) {
        _mint(_userAddress, _value);
        return true;
    }

    function redeemGold(address _userAddress, uint256 _value) external onlySupplyController returns (bool) {
    ...
    function delegateTransfer(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(this, delegator, recipient, amount, networkFee, nonces[delegator]++)
        );
    ...
    function _transferGIFT(address sender, address recipient, uint256 amount) internal virtual returns (bool) {
        ...
        if (!taxManager.isExcludedFromOutboundFees(sender) && !taxManager._isLiquidityPool(recipient)) {
        ...
        if (!taxManager.isExcludedFromInboundFees(recipient) && !taxManager._isLiquidityPool(sender)) {
