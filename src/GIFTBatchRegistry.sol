    ...
    UUPSUpgradeable
{
    ...
    event LegacySupplyAcknowledged(
        uint256 indexed batchId,
        uint256 amount,
        string reason,
        bytes32 docHash
    ...
    function getBatchMeta(
        uint256 batchId
    ) external view returns (BatchMeta memory) {
        return _requireBatch(batchId);
    }

    function leafConsumed(
        uint256 batchId,
        bytes32 leafHash
    ...
    function consume(
        ...
        require(
            MerkleProof.verifyCalldata(proof, b.root, leafHash),
            "Registry: bad proof"
        );
    ...
    function _requireBatch(
        uint256 batchId
    ) internal view returns (BatchMeta storage b) {
        b = _batches[batchId];
