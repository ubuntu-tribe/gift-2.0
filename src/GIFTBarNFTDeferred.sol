...
contract GIFTBarNFTDeferred is ERC721, Ownable, ReentrancyGuard {
    ...
    IGIFTBatchRegistry public immutable registry;
    ...
    event BarsMinted(uint256 indexed batchId, bytes32 indexed leafHash, uint256 reserveId, uint256 units, address to, uint256 fromTokenId, uint256 toTokenId);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(
            b, "/", _u(info.batchId), "/", _hex(info.leafHash), "/", _u(tokenId), ".json"
        ));
    }

    function remainingUnits(IGIFTBatchRegistry.LeafInput calldata leaf, bytes32[] calldata proof)
        external
        view
    ...
    function _capacityUnits(uint256 batchId, bytes32 leafHash, uint256 quantitySmallest, uint256 fineWeightMg)
        internal
        view
        returns (uint256)
    {
        if (capacityMode == CapacityMode.LEAF_MAX) {
            ...
            // STRICT: cap by ERC-20 minted from this leaf
            uint256 consumedSmall = registry.leafConsumed(batchId, leafHash);
            // Convert proportionally: units = floor(consumedSmall * fineWeightMg / quantitySmallest / unitMg)
